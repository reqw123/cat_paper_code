...
#  設定區  ── 修改這裡，不需要命令列參數
# ═══════════════════════════════════════════════════════
OLD_MODEL_PATH   = r"C:\ai_project\cat_pose\aug_8.pt"
NEW_MODEL_PATH   = r"C:\ai_project\cat_pose\aug_8_2.pt"
...
def write_final_summary_log(log_dir, old_name, new_name, total_videos,
    ...
    lines = [
        ...
        "Old Model Score:", f"{old_comp:.2f}", "",
        "New Model Score:", f"{new_comp:.2f}", "",
        "Overall Improvement:", f"{overall_score:+.2f}%", "",
        "Decision:", "NEW MODEL WINS" if overall_score > 0 else "OLD MODEL WINS", "",
        "Confidence:", f"{ci:+.2f}%", "",
        "Jitter:", f"{ji:+.2f}%", "",
...
def write_html_report(report_dir, old_name, new_name, total_videos,
    ...
    tti = improvement(agg_old["tail_tip_missing_ratio"], agg_new["tail_tip_missing_ratio"], False)
    best_b, best_s, worst_b, worst_s = best_worst_behavior(class_old, class_new)
    is_better = overall_score > 0
    vc = "#4CAF50" if is_better else "#F44336"
    ...
    rec_bg  = "#e8f5e9" if is_better else "#ffebee"
    rec_msg = ("✅ <strong>Replace old model with new model.</strong>"
               if is_better else
               "❌ <strong>Keep old model. New model does not show sufficient improvement.</strong>")
...
<div class="models">
  <div class="mbox" style="border-left:4px solid {COLOR_OLD}">
    ...
    <div style="font-size:12px;color:#888">Composite Score</div>
  </div>
...
<div class="verdict">{"NEW MODEL IS BETTER" if is_better else "NEW MODEL IS WORSE"}
<br><span style="font-size:16px">Overall Improvement: {overall_score:+.2f}%</span></div>

<h2>Key Metrics</h2>
<div class="cards">
{card("Confidence", ci, f"{agg_old['mean_confidence']:.3f}", f"{agg_new['mean_confidence']:.3f}")}
{card("Jitter  ↓ better", ji, f"{agg_old['mean_jitter_px']:.2f}px", f"{agg_new['mean_jitter_px']:.2f}px")}
{card("Missing Ratio  ↓ better", mi, f"{agg_old['missing_ratio']*100:.1f}%", f"{agg_new['missing_ratio']*100:.1f}%")}
{card("Tail Tip Missing  ↓ better", tti, f"{agg_old['tail_tip_missing_ratio']*100:.1f}%", f"{agg_new['tail_tip_missing_ratio']*100:.1f}%")}
{card("Best Behavior", best_s, best_b.capitalize(), "improved most")}
{card("Worst Behavior", worst_s, worst_b.capitalize(), "needs attention")}
</div>
...
def main():
    ...
    logger.info(_vlj("結論", W_LBL) + f"{Path(new_path).name}  相較  {Path(old_path).name}  →  {verdict}")
    logger.info(SEP3)
