...
>
> 內容以**實際原始碼現況**為準（校對日期：2026-07-15），刻意不重複程式碼細節，只給「地圖」——路徑、職責、彼此的呼叫關係。
>
...
| `cat_monitoring_system/tools/train_data/0_dataset_collect.py` | 骨架資料集收集與手動標注工具 |
...
> ⚠️ `run_keypoint_verification.bat` 呼叫的 `1_check_keypoint_importance.py` 目前已不存在於專案中（僅剩 `__pycache__` 殘留的 `.pyc`），此 `.bat` 目前已失效、待清理或補回對應腳本。
