...
> **ST-GCN 實際僅使用前 14 個關鍵點**（`STGCNConfig.NUM_JOINTS = 14`）。YOLO-Pose 仍輸出完整 17 點，但推論管線（`behavior_classifier.py`）會截斷尾巴三點（14 tail_base / 15 tail_mid / 16 tail_tip）後才送入 ST-GCN；此截斷邏輯與訓練設定 `stgcn_config.yaml` 的 `NUM_JOINTS` 一致，屬消融實驗保留下來的最終設計（捨棄尾巴點以提升穩定性）。
...
| `skeleton_quality_assessment.evaluate_window()` | 「GCN 分類為主、幾何判斷為輔」雙重判定（見 3.6）：3 項骨架幾何指標判斷該推論窗口是否可信，不可信時覆蓋分類結果為 LOW_CONF；總開關 `SQAConfig.ENABLE_SQA_DUAL_JUDGMENT` 預設關閉 |
...
| GPT API | 讀取 `C:\a\noda_data.csv`，以 gpt-4.1-mini 生成繁體中文健康分析報告；透過 **Facebook Messenger Webhook**（`/messengerwebhook`）接收指令（哈基米觸發報告 / `/camera` / `/status` / `/help`）；以獨立 Node-RED Tab「CSV AI分析系統」實作（預設 disabled，需手動啟用），定義於 `GPT 健康報告.json` |

> ⚠ **已知問題**：`GPT 健康報告.json` 中 `/status` 指令節點呼叫 Flask 的 `http://<python_ip>:5000/status`，但 `server/routes.py` 目前未定義 `/status` 路由（僅有 `/stream`、`/snapshot`、`/video_clip`、`/api/behavior_history`、`/api/overlay`、`/`），該指令會持續失敗，需補上路由或移除該 Node-RED 節點。
...
**校準現況**：門檻值目前僅用少量影片（`tools/test_bone_length_stability.py` 模式3 建立的正常骨架基準 + 個位數支已知異常影片）粗略校準，尚未經過大量資料驗證，因此正式整合到推論管線 `processors/frame_processor.py` 後，總開關 `config.py::SQAConfig.ENABLE_SQA_DUAL_JUDGMENT` **預設關閉**，需要更多影片驗證覆蓋規則後再手動開啟。
...
| 骨架品質雙重判定（SQA） | 🔄 初版完成，待校準 | 已整合進推論管線但預設關閉，需更多影片驗證門檻後再啟用（見 3.6） |
| 論文撰寫 | 🔄 進行中 | |
...
| 預設特徵模式 | xy_conf_v_bone（7ch） | `STGCNConfig.FEATURE_MODE`（與部署 checkpoint `076_xy_conf_v_bone_att_on.pth` 一致） |
...
- 骨架品質雙重判定（SQA）門檻校準：目前門檻僅用少量影片粗估，需擴大到多種行為類別、多隻貓的正常/異常影片，用 `tools/test_bone_length_stability.py` 模式1/3 持續驗證 flag rate 後，再正式啟用 `SQAConfig.ENABLE_SQA_DUAL_JUDGMENT`
