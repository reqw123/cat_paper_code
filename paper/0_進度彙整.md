...
- 每隔 `WINDOW_STRIDE = 16` 幀觸發一次 ST-GCN 推論（推論步長，定義於 `config.py`）
...
| GPT API | 讀取 `C:\a\noda_data.csv`，以 gpt-4.1-mini 生成繁體中文健康分析報告；透過 **Facebook Messenger Webhook**（`/messengerwebhook`）接收指令（哈基米觸發報告 / `/camera` / `/status` / `/help`）；以獨立 Node-RED Tab「CSV AI分析系統」實作（預設 disabled，需手動啟用） |
...
| 預設特徵模式 | xy（2ch） | `STGCNConfig.FEATURE_MODE` |
