...
YOLO_MODEL_PATH = r"C:\AI_Project\cat_pose\train_best.pt"
...
# Debug: print body_len per processed frame for threshold tuning
PRINT_BODY_LEN = True
...
# body_len 較短時，套用固定放大倍率，避免區域過小造成漏判。
# 根據實際量測，當 body_len 小於此值時啟動短尺度補強
NOSE_CONTACT_SHORT_BODY_LEN_PX = 550.0
# 補強倍率（>1 表示放大）；例如 1.5 表示放大 50%
NOSE_CONTACT_SHORT_BODY_BOOST_SCALE = 1.8
NOSE_CONTACT_SHORT_BODY_SCALE_MIN = 1.0
...
def compute_limb_joint_targets(kpts, kpt_conf, body_len):
    ...
    # 當 body_len 太短時，套用短尺度補強（使用與鼻子相同的門檻與倍率）
    limb_scale = 1.0
    if base_len < float(NOSE_CONTACT_SHORT_BODY_LEN_PX):
        limb_scale = float(NOSE_CONTACT_SHORT_BODY_BOOST_SCALE)
        limb_scale = float(np.clip(limb_scale, NOSE_CONTACT_SHORT_BODY_SCALE_MIN, 10.0))

    paw_radius = max(
        1e-6,
        base_len * float(LIMB_PAW_CIRCLE_RADIUS_RATIO) * float(LIMB_CONTACT_SCALE) * limb_scale,
    )
    limb_targets = []
...
def compute_limb_strip_targets(kpts, kpt_conf, body_len):
    ...
    half_width = max(
        1e-6,
        base_len * float(LIMB_STRIP_HALF_WIDTH_RATIO) * float(LIMB_CONTACT_SCALE) * limb_scale,
    )
    edge_gap = max(
        0.0,
        base_len * float(LIMB_STRIP_EDGE_GAP_RATIO) * float(LIMB_CONTACT_SCALE) * limb_scale,
    )
    paw_radius = max(
        1e-6,
        base_len * float(LIMB_PAW_CIRCLE_RADIUS_RATIO) * float(LIMB_CONTACT_SCALE) * limb_scale,
...
def compute_head_body_target_geometry(kpts, kpt_conf):
    ...
    short_body_scale = 1.0
    if body_len < float(NOSE_CONTACT_SHORT_BODY_LEN_PX):
        short_body_scale = float(NOSE_CONTACT_SHORT_BODY_BOOST_SCALE)
        short_body_scale = float(np.clip(short_body_scale, NOSE_CONTACT_SHORT_BODY_SCALE_MIN, 10.0))
...
def main():
    ...
    while not stop_all:
        ...
        while True:
            ...
            display_det_only = display.copy() if DISPLAY_WINDOW else None
            ...
            if not front_view_guard and full_ui_mode:
                ...
                # Debug: optionally print/log body_len for tuning
                if target_geom is not None and PRINT_BODY_LEN:
                    try:
                    ...
                    if BODY_LEN_LOG_PATH:
                        try:
                            with open(BODY_LEN_LOG_PATH, "a", newline="") as _f:
                                _f.write(f"{current_video_idx+1},{processed_frame_idx},{bl:.6f}\n")
                        except Exception:
                            pass
            ...
            display_no_info = display.copy() if DISPLAY_WINDOW else None
            ...
            if DISPLAY_WINDOW:
                ...
                if action == "enter_pause":
                    def _build_pause_frame():
                        ...
                        elif show_overlay_info and display is not None:
                            source = display
                        elif display_no_info is not None:
                            source = display_no_info
                        else:
                            source = display
