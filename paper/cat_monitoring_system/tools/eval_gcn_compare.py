...
def infer_num_joints(model_path: str):
    """從 checkpoint 的空間圖 adjacency buffer（'stgcn_layers.0.sgc.A'，shape
    K×V×V）推斷 num_joints（V），跟 models/stgcn_model.py::CatBehaviorSTGCN
    載入 checkpoint 時的自動偵測用同一個 key，讓 14/17 關節的模型可以在同一次
    比較裡混用，不需要另外指定。找不到該 buffer（例如非常舊的 checkpoint）時
    回傳 17（沿用預設值，維持原本行為）。"""
    if not Path(model_path).exists():
        return 17
    ck = torch.load(model_path, map_location='cpu')
    sd = ck.get('model_state_dict', ck) if isinstance(ck, dict) else ck
    if isinstance(sd, dict):
        adj = sd.get('stgcn_layers.0.sgc.A')
        if adj is not None:
            return int(adj.shape[-1])
    return 17
...
def main():
    ...
    for i, (cfg, lbl, v) in enumerate(zip(models_cfg, labels, num_joints_list)):
        print(f"  Model {chr(65 + i)} : {lbl}  (EMA α={cfg['ema_alpha']}  T={cfg['seq_len']}  V={v})")
