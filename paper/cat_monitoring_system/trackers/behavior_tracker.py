...
class ImprovedBehaviorTracker:
    ...
    def update(self, behavior_id, activity_value):
        ...
        if record_this and self.current_behavior is not None:
            ...
            self.current_behavior = behavior
            self.behavior_start_time = now
        self.activity_window.append({"time": now, "activity": activity_value, "weight": duration if duration > 0 else BehaviorTrackingConfig.DEFAULT_ACTIVITY_WEIGHT})
    ...
    def get_alerts(self):
        ...
        if total_time > 0 and normal_time < BehaviorTrackingConfig.LOW_ACTIVITY_TIME_THRESHOLD_SECONDS:
